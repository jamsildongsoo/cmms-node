...
export class ApprovalController {
  ...
  async updateApproval(
    ...
    const { companyId, userId } = getTenantContext();
    return this.approvalService.updateApproval(companyId, id, request, userId);
  }

  @Delete(':id')
  async deleteApproval(@Param('id') id: string): Promise<void> {
    const { companyId, userId } = getTenantContext();
    await this.approvalService.deleteApproval(companyId, id, userId);
  }
  ...
  // 결재 권한은 모듈 권한이 아니라 현재 결재선의 담당자 여부로 판단한다.
