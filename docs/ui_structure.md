...
주의사항:
*   최초 로그인 또는 비밀번호 만료 사용자는 `PasswordChangeNotice` 모달에서 비밀번호 변경을 안내받으며, 안내를 닫고 계속 사용할 수 있습니다.
...
*   별도 사용자 동작이 필요 없는 성공, 실패, 안내 메시지는 `sonner`의 toast로 표시합니다.
*   확인이나 취소가 필요한 작업은 공통 React 확인 대화상자를 사용합니다.
*   반려 사유처럼 추가 입력이 필요한 작업은 공통 입력 대화상자 또는 해당 화면의 인라인 입력 영역을 사용합니다.
*   현재 입력을 수정해야 진행할 수 있는 오류는 해당 필드 또는 폼 안에 인라인으로 표시할 수 있습니다.
*   브라우저 기본 `alert`, `confirm`, `prompt`와 화면별 임시 메시지 구현은 사용하지 않습니다.

### 6.2 문서 출력

*   설비·재고 목록은 별도 HTML 문서를 만들고 `window.print()`를 호출합니다.
*   예방점검, 작업지시서, 작업허가서, 결재 문서는 업무별 React 출력 컴포넌트와 공통 출력 창 레이아웃을 사용합니다.
*   목록의 문서번호를 선택하면 출력 화면으로 이동하며, 출력 화면에는 `출력` 버튼만 제공합니다.
*   수정 화면에는 인쇄 또는 PDF 저장 버튼을 두지 않습니다.
*   회사명, 출력일시 등 상단 정보는 공통 `PrintHeader`를 사용합니다.
*   PDF 저장은 별도 기능으로 제공하지 않고 브라우저의 인쇄 대상에서 PDF 프린터를 선택하도록 합니다.
